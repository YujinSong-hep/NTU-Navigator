...
# =========================================================
# 1. Weather and environment structure
# =========================================================
...
def resolve_vpr_room_id_from_label(label: str, catalog_labels: Optional[List[str]] = None) -> Optional[int]:
    ...
    # Only allow fixed-name mapping; no compatibility for direct numeric parsing.
...
def get_inventory_rows_app_order() -> List[Tuple[int, int, str, int]]:
    # Output: [(display index 1-8, app3 room ID, fixed name, feature count)]
...
def build_structure(is_raining_at_all: bool = False):
    ...
    D, H, W = 2, 16, 80
    structure = np.ones((D, H, W), dtype=int) # Start fully blocked (wall shading)

    # ==========================================
    # Z=1 (Figure 1, upper floor B1)
    # ==========================================
    structure[1, 12:15, 2:20] = 0   # Left base hall
    structure[1, 9:12, 6:16] = 0    
    structure[1, 6:9, 11:16] = 0  
    structure[1, 1:15, 20:24] = 0  
    # Core: ultra-narrow straight top corridor
    structure[1, 1:3, 24:80] = 0

    # ==========================================
    # Z=0 (Figure 2, lower floor B2)
    # ==========================================
    structure[0, 12:15, 2:20] = 0   # Left base hall
    structure[0, 9:12, 6:16] = 0    
    structure[0, 6:9, 11:16] = 0  
    # Core: ultra-narrow straight bottom corridor
    structure[0, 13:15, 20:80] = 0

    # ==========================================
    # Connectivity facilities
    # ==========================================
    structure[0:2, 11:12, 14:15] = 2 # Elevator (yellow block)
    structure[0:2, 11:12, 8:9] = 3   # Left stair (triangle)

    # Physical connectivity layer: hidden right-side stair shaft (for AI routing only, not displayed)
...
def extract_rooms(structure: np.ndarray) -> List[Room]:
    ...
    rooms = []
    # B1 upper floor
    rooms.append(Room(1, 1, 12, 10, 10))  # 1) Left open area
    rooms.append(Room(2, 1, 1, 38, 10))   # 2) Top corridor
    rooms.append(Room(3, 1, 1, 56, 10))   # 3) Top corridor
    rooms.append(Room(4, 1, 1, 72, 10))   # 4) Right side of top corridor

    # B2 lower floor
    rooms.append(Room(5, 0, 13, 10, 10))  # 5) Left open area
    rooms.append(Room(6, 0, 14, 24, 10))  # 6) Bottom corridor
    rooms.append(Room(7, 0, 14, 56, 10))  # 7) Bottom corridor
    rooms.append(Room(8, 0, 14, 72, 10))  # 8) Right side of bottom corridor
    return rooms
...
# =========================================================
# 3. Classic pathfinding and plotting (fixed and integrated)
# =========================================================
def dijkstra_route(structure, start, end, elev_penalty, is_raining):
    ...
    while pq:
        ...
        for action in valid_moves(cur, structure):
            ...
            # Block the right stair during rain
...
def render_labeled_floors(structure, rooms, path=None, start=None, end=None, is_raining=False):
    ...
    if path:
        for z, y, x in path: 
            # Hide route segments: the red line should never pass through large shaded areas
            if x >= 74:
    ...
    for z in range(d):
        ...
        # Extract the physical map for display
        cls = np.ones((h, w), dtype=int) 
        cls[structure[z] != 1] = 0 
        
        # Visual hard erase: cover the right-side routing corridor with shading
        if z == 1:
            cls[3:16, 74:80] = 1 # B1: fully black below the corridor
        else:
            cls[0:13, 74:80] = 1 # B2: fully black above the corridor
...
# =========================================================
# 4. Streamlit UI (restore original sidebar layout)
# =========================================================
def main():
    ...
    # Build structure based on weather status
    ...
    # Keep decimal coordinates in UI/business layer; pathfinders snap internally to nearest walkable grid.
    ...
    # --- Restore left-right columns on main page ---
